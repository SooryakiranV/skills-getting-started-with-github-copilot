    ...
    try {
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantsMarkup = participants.length > 0
          ? `<ul class="participants-list">${participants.map(email => `<li><span class="participant-name">${email}</span><button class="participant-remove" type="button" data-activity="${encodeURIComponent(name)}" data-email="${encodeURIComponent(email)}" aria-label="Remove ${email}">✖</button></li>`).join('')}</ul>`
